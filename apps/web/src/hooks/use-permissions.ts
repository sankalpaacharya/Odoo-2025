...
const ROUTE_PERMISSIONS: Record<string, { module: string; permission: string }> = {
    ...
    const fetchRoleAndPermissions = async () => {
      try {
        ...
        if (!employeeResponse.ok) {
          console.warn("Failed to fetch employee data, status:", employeeResponse.status);
          setLoading(false);
          return;
        }
        ...
        console.log("Fetched employee role:", role);
        setUserRole(role);

        // Now fetch permissions for this role
        const permissionsResponse = await fetch(`${API_URL}/api/permissions/${role}`, {
          credentials: "include",
        });
        ...
        if (permissionsResponse.ok) {
          ...
          console.warn("Failed to fetch permissions, status:", permissionsResponse.status);
        }
      } catch (error) {
