...
export function StatusBadge({ status }: StatusBadgeProps) {
  const normalizedStatus = status
    .toUpperCase()
    .replace(/ /g, "_") as keyof typeof STATUS_STYLES;
  const style =
    STATUS_STYLES[normalizedStatus] ||
    "bg-gray-500/10 text-gray-700 dark:text-gray-400 border-transparent hover:bg-gray-500/20";
...
export function EmployeeAvatar({
  ...
  return (
    <div
      className={`${sizeClasses[size]} shrink-0 rounded-full bg-muted flex items-center justify-center font-light text-muted-foreground`}
    >
      {initials}
    </div>
  );
